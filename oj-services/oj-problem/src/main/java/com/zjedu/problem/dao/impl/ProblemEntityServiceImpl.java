...
public class ProblemEntityServiceImpl extends ServiceImpl<ProblemMapper, Problem> implements ProblemEntityService
{
    ...
    public boolean adminAddProblem(ProblemDTO problemDto)
    {
        ...
        for (Language language : problemDto.getLanguages())
        {
            if (language.getId() == null)
            {
                ...
                queryWrapper.eq("name", language.getName());
                queryWrapper.eq("description", language.getDescription());
                language.setId(languageEntityService.getOne(queryWrapper).getId());
            }
    ...
    public boolean adminUpdateProblem(ProblemDTO problemDto)
    {
        ...
        map.put("pid", pid);
        //查询出原来题目的关联表数据
        List<ProblemLanguage> oldProblemLanguages = problemLanguageEntityService.listByMap(map);
        List<ProblemCase> oldProblemCases = problemCaseEntityService.listByMap(map);
        List<CodeTemplate> oldProblemTemplate = codeTemplateEntityService.listByMap(map);
        List<ProblemTag> oldProblemTags = problemTagEntityService.listByMap(map);
