...
import com.zjedu.judge.common.exception.CompileError;
import com.zjedu.judge.common.exception.SubmitError;
import com.zjedu.judge.common.exception.SystemError;
import com.zjedu.judge.dao.JudgeCaseEntityService;
import com.zjedu.judge.dao.JudgeEntityService;
import com.zjedu.judge.judge.entity.LanguageConfig;
import com.zjedu.judge.judge.entity.Pair_;
...
//                    .toString().replace("\\", "/");
                                                ...
                                                String judgeCaseMode)
    {
        ...
        testCaseResultList.forEach(jsonObject ->
        {
            ...
            if (StringUtils.hasText(msg) && !Objects.equals(status, Constants.Judge.STATUS_COMPILE_ERROR.getStatus()))
            {
        ...
        if (!addCaseRes)
        {
            log.error("题号为：{}，提交id为：{}的各个测试数据点的结果更新到数据库操作失败", problem.getId(), judge.getSubmitId());
        }
    ...
    private String getUserFileName(String language)
    {
        ...
        {
            case "PHP" -> "main.php";
            case "JavaScript Node", "JavaScript V8" -> "main.js";
            default -> "main";
        };
    }
