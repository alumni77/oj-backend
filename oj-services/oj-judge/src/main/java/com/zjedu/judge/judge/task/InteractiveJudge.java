package com.zjedu.judge.judge.task;

import cn.hutool.core.util.NumberUtil;
import cn.hutool.json.JSONArray;
import cn.hutool.json.JSONObject;
import com.zjedu.judge.common.exception.SystemError;
import com.zjedu.judge.judge.AbstractJudge;
import com.zjedu.judge.judge.SandboxRun;
...
import com.zjedu.judge.judge.entity.SandBoxRes;
...
public class InteractiveJudge extends AbstractJudge
{
    ...
    public JSONObject checkMultipleResult(SandBoxRes userSandBoxRes, SandBoxRes interactiveSandBoxRes, JudgeDTO judgeDTO, JudgeGlobalDTO judgeGlobalDTO)
    {
        ...
        } else
        {
            ...
            if (StringUtils.hasText(spjErrMsg))
            {
                errMsg.append(spjErrMsg).append(" ");
            }
            if (interactiveSandBoxRes.getExitCode() != 0 && StringUtils.hasText(interactiveSandBoxRes.getStderr()))
            {
        ...
        // 记录该测试点的错误信息
        if (StringUtils.hasText(errMsg.toString()))
        {
    ...
    private JSONObject checkInteractiveRes(SandBoxRes interactiveSandBoxRes)
    {
        ...
        // 获取跑题用户输出或错误输出
        if (StringUtils.hasText(interactiveSandBoxRes.getStderr()))
        {
