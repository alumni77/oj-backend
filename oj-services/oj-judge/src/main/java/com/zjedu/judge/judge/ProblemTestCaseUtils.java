...
public class ProblemTestCaseUtils
{
                                          ...
                                          String judgeCaseMode) throws SystemError
    {
        ...
        // 可能是zip上传记录的是文件名，
        if (!StringUtils.hasText(problemCases.get(0).getInput())
                || !StringUtils.hasText(problemCases.get(0).getOutput())
        ...
        } else
        {
            ...
            List<HashMap<String, Object>> testCases = getHashMaps(problemCases);

            return initTestCase(testCases, problemId, version, judgeMode, judgeCaseMode);
        }
    }

    private static List<HashMap<String, Object>> getHashMaps(List<ProblemCase> problemCases)
    {
