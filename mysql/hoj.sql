...
CREATE TABLE `judge` (
  ...
  `gmt_create` datetime DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`submit_id`,`pid`,`display_pid`,`uid`),
  KEY `pid` (`pid`),
  KEY `uid` (`uid`),
...
drop index problem_id on problem;

create unique index problem_id
